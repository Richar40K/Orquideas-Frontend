...
export default function ListaDeViajes({ viajesToShow, hasSearched, isLoading, error }: ListaDeViajesProps) {
  const router = useRouter();

  if (isLoading) {
    return <div className="text-center p-6 text-blue-700">Cargando viajes...</div>;
  }
  ...
  if (hasSearched && viajesToShow.length === 0) {
    return <div className="text-center p-6 text-gray-600">No se encontraron viajes para los criterios seleccionados.</div>;
  }

  if (!hasSearched && viajesToShow.length === 0) {
    return <div className="text-center p-6 text-gray-600">No hay viajes programados disponibles en este momento.</div>;
  }

  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-6 text-blue-900 text-center">Resultados de tu Búsqueda</h2>
      <div className="space-y-5">
        {viajesToShow.map((viaje) => (
          <div
            key={viaje.id}
